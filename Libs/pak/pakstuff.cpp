...
struct PK3FileInfo
{
  ...
  PK3FileInfo() {  memset(this, 0, sizeof(PK3FileInfo));  }
...
public:
  ...
  void Log(const char *pFormat, ...)
  {
	...
	if ((m_pFile)) fprintf(m_pFile, pFormat, arg_ptr);
    va_end(arg_ptr);
  }
};
...
int GenHashVal(const char *s) {  // simple hash function.  May need to be replaced with a better one.
	int hash = 0;
...
//
// TTimo: don't return only filename, return relative path to the search pattern
// ex: if pattern == "sfx/*.tga" and we find some "sfx/compscreen/letters1.tga" put "compscreen/letters1.tga"
// G_Dewan: hash code
boolean LoadPK3FileList(FILELIST **filelist, const char *pattern)
{
  ...
  for(hash_val = 0; hash_val < 0x100; hash_val++)
  {
	  PK3List *p = g_PK3Files[hash_val].Next();
	  while (p != NULL)
	  {
		// qualify the path
		...
		{
      strcpy( cWork, strstr( pKey->m_pName, cPath )+strlen(cPath) );
...
int PakLoadAnyFile(const char *filename, void **bufferptr)
{
  ...
  if (g_bPK3)
  {
	...
	int hash_val = GenHashVal(infoFind.m_pName);

    PK3List *pList = g_PK3Files[hash_val].Find(&infoFind);
    if (pList)
    {
      pInfo = pList->Ptr();
  ...
  if (zFile != NULL)
  {
    ...
    while (nStatus == UNZ_OK)
    {
      ...
      if (strstr(cWork, ".") != NULL)
      {
        ...
        pInfo->m_zFile = *zFile;
		hash_val = GenHashVal(pInfo->m_pName);
        g_PK3Files[hash_val].Add(pInfo);
      }
