...
void CreateEntityFromName(char* pName, brush_t* pBrush)
{
  ...
  // see SF bug 105383
  if (stricmp(pName, "light") == 0)
  {
    CLightPrompt prompt;
    prompt.m_iIntensity = g_PrefsDlg.m_iLastLightIntensity;
...
//    prompt.GotoDlgCtrl( pWnd );
    ...
    {
      g_PrefsDlg.m_iLastLightIntensity = prompt.m_iIntensity;
      char buf[10];
      sprintf( buf, "%d", prompt.m_iIntensity );
      SetKeyValue( petNew, "light", buf );
    }
  }
...
BOOL FilterBrush(brush_t *pb)
{
	...
	if (g_qeglobals.d_savedinfo.exclude & EXCLUDE_CAULK)
  {
    ...
    // if not don't hide the whole brush, proceed on a per-face basis (Cam_Draw)
		// NOTE: we could have rendering parameters for each brush and update them at Brush_Build for some speedup
    face_t *f;
    f=pb->brush_faces;
    while (f)
    {
	...
	if (g_qeglobals.d_savedinfo.exclude & EXCLUDE_HINT)
  {
	...
	{
		// see EXCLUDE_CAULK above
		face_t *f;
		f=pb->brush_faces;
		while (f)
		{
			if (!strstr(f->texdef.name, "clip"))
				break;
			f = f->next;
		}
		if (!f)
...
BOOL CXYWnd::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt )
{
	...
	if (zDelta<0) OnViewZoomout();

	return true;
}

void CXYWnd::OnViewZoomin() 
{
  if (Active())
  {
    SetScale(Scale() * 5.0 / 4);
    if (Scale() > 16)
      SetScale(16);
  }
...
void CXYWnd::OnViewZoomout() 
{

  if (Active())
  {
    SetScale(Scale() * 4.0 / 5);
    if (Scale() < 0.1)
      SetScale(0.1);
  }
