...
// JONATHAN Check this
// Possible to do model scale code here too
void TrackMD3Angles(entity_t *e, const char *key, const char *value)
{

#if 0
	// JONATHAN:: Better off getting the mins/max from the transformed vertexes
  if (strcmpi(key, "modelscale") == 0)
  {
	 vec_t oldScale = FloatForKey(e,"modelscale");
	 vec_t newScale = atof(value);
	 if (oldScale != newScale)
	 {
...

		 {
			 ...
			 VectorSubtract(e->md3Class->maxs,e->origin,temp_maxs);
			 VectorSubtract(e->md3Class->mins,e->origin,temp_mins);

			 if (oldScale != 1)
			 {
				 VectorScale(temp_maxs, (1/oldScale), temp_maxs);
				 VectorScale(temp_mins, (1/oldScale), temp_mins);
			 }
			
			 if (newScale != 1)
			 {
				 VectorScale(temp_maxs, newScale, temp_maxs);
				 VectorScale(temp_mins, newScale, temp_mins);
			 }

			 VectorAdd(temp_maxs, e->origin, e->md3Class->maxs);
			 VectorAdd(temp_mins, e->origin, e->md3Class->mins);
			 
			 Brush_Resize(e->brushes.onext,	e->md3Class->mins, e->md3Class->maxs);	 
		 }
	 }
  }
#endif
  ...
  int n = 0;
	do
	{
		...
		if (!strcmp (token, "{") )
		{
			...
			{
        //++timo this may mean we bumped into a BP conversion problem and user canceled load
        // FIXME: remove when mixing the two formats
	...
	{	// fixed size entity
		...
		if (e->nShowFlags & ECLASS_MISCMODEL)
		{
			...
			if (p != NULL && strlen(p) > 0)
			{
				...
				{
					// JONATHAN: Load md3 bounding box set size
					// I'm not happy with this
#if 0
					vec_t sc = FloatForKey(ent,"modelscale");
					if (sc>0)
					{
						VectorScale(ent->md3Class->mins, sc, ent->md3Class->mins);
						VectorScale(ent->md3Class->maxs, sc, ent->md3Class->maxs);						
					}
#endif
					// create a custom brush
...
void Entity_Write (entity_t *e, FILE *f, qboolean use_region)
{
  ...
  // TTimo: undo produces bogus worldspawn entities, get rid of them
  if ((!strcmp(ValueForKey (e, "classname"), "worldspawn")) && (e!=world_entity))
  {
    MessageBox( g_qeglobals.d_hwndMain, "Dropping a parasite worldspawn entity.", NULL, MB_OK );
    return;
  }
