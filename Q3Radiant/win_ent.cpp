...
void SetInspectorMode(int iType)
{
	...
	if (iType == W_GROUP)
		MessageBox( g_qeglobals.d_hwndMain, "Brush grouping is not functional yet", NULL, MB_OK | MB_ICONWARNING );
...
void AddProp()
{
  ...
  if (!strcmp(key, "classname") && !strcmp(value, "worldspawn"))
		{
    MessageBox( g_qeglobals.d_hwndMain, "Cannot change \"classname\" key back to worldspawn.", NULL, MB_OK );
		return;
		}
	...
	// JONATHAN: If value was modelscale then the bounding box needs updating
#if 0
	if (!strcmp(key,"modelscale") || !strcmp(key,"angles") )
	{
		if (multiple_entities)
		{
			brush_t	*b;
			for (b=selected_brushes.next ; b != &selected_brushes ; b=b->next)
				if (b->owner->md3Class->model)
				{
				}
		}
		else
...
//  CString strBasePath = ValueForKey(g_qeglobals.d_project_entity, "basepath");
  // DIRTY: some project file are not using short path names like you would expect
  //   so here you end up not being able to find the basepath in the model path
  //   a quick fix that doesn't have much consequences is to do an additional BuildShortPathName here
  char shortPath[_MAX_PATH];
  strcpy_s(shortPath, _MAX_PATH,ValueForKey( g_qeglobals.d_project_entity, "basepath") );
  //BuildShortPathName( ValueForKey( g_qeglobals.d_project_entity, "basepath"), shortPath, _MAX_PATH );
  CString strBasePath = shortPath;
  ...
  if (dlgFile.DoModal() == IDOK)
  {
    ...
    if (n == 0)
    {
      ...
      Sys_Printf("WARNING: could not extract the base path from .md3 path\n");
...
// TTimo: added a BuildShortPathName after getting path to the .md3
// Radiant failed to extract the base path if there's a difference between full path and old short dos version
void AssignModel()
{
//  CString strBasePath = ValueForKey(g_qeglobals.d_project_entity, "basepath");
  // DIRTY: some project file are not using short path names like you would expect
  ...
  CFileDialog dlgFile(TRUE, NULL, NULL, OFN_OVERWRITEPROMPT, "Model files (md3 ase obj 3ds)|*.md3;*.ase;*.obj;*.3ds||", g_pParentWnd);
  dlgFile.m_ofn.lpstrInitialDir = strPath;
  if (dlgFile.DoModal() == IDOK)
  {
	SendMessage(hwndEnt[EntKeyField], WM_SETTEXT, 0, (LPARAM)"model");
	CString str = dlgFile.GetPathName().GetBuffer(0);
		// Jonathan TODO:: Should I remove this line? What consiquence would it have?
		// I'm removing it because win7 doesn't behave the same as xp when dealing with dos-8 names
			//char auxBuf[1024];
			//BuildShortPathName( str.GetBuffer(0), auxBuf, 1024 );
			//str = auxBuf;
		// JN: End edit
    str.MakeLower(); 
    strBasePath.MakeLower();
	...
	else Sys_Printf("WARNING: could not extract the base path from .md3 path\n");

	// Jonathan: Tempory debug output
	//Sys_Printf(strBasePath.GetBuffer(0));
	//Sys_Printf("\n");
  ...
  LPNMHDR lpnmh = NULL;
    ...
    switch (uMsg)
    {
      ...
      case WM_CHAR :
      {
        ...
        //else
			// JONATHAN: Disable brush grouping feature (it has never worked)
        //if (c == 'G')
