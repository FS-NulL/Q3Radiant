...
face_t	*Face_FullClone (face_t *f)
{
	...
	if (f->face_winding)
		...
		n->face_winding = NULL;
	n->pShader = QERApp_Shader_ForName( n->texdef.name );
	n->pShader->IncRef();
  ...
  do
	{
      ...
      //++timo FIXME: we'll soon allow mixing of the two formats in the same map
			if (!g_qeglobals.m_bBrushPrimitMode)
			{
  			// check the map is not mixing the two kinds of brushes
				if (g_qeglobals.bOldBrushes)
					Sys_Printf("Warning: standard format and brush primitives in the same file are not allowed ( Brush_Parse )\n");
        // prompt the user once about converting (when conversion has begun this flag is true)
        if (!g_qeglobals.bNeedConvert)
        {
          // we didn't turn the conversion flag on yet, it's time to ask the user
          if (MessageBox( g_qeglobals.d_hwndMain, "This map was saved using brush primitives format, and your project settings use the standard format.\nDo you want to convert it back to standard format?\nNOTE: due to limitations of the standard format, some textures may be misplaced after conversion.", "Conversion required", MB_YESNO ) == IDNO )
          {
            // we need a global flag to cancel the whole map loading process
            g_bCancel_Map_LoadFile = true;
            return NULL;
          }
        }
        // set the "conversion needed" flag (conversion actually happens in Brush_Build
				g_qeglobals.bNeedConvert=true;
			}
			...
			else
      {
        ...
        {
          // we didn't turn the conversion flag on yet, it's time to ask the user
          if (MessageBox( g_qeglobals.d_hwndMain, "This map was saved using standard format, and your project settings use the new \"brush primitives\" format. Do you want to convert it?\nNOTE: Brush primitives is an experimental feature, backup your work first. Next versions of Radiant will allow mixing the two formats in the same maps for a smooth transition.", "Conversion required", MB_YESNO ) == IDNO )
          {
            // we need a global flag to cancel the whole map loading process
            g_bCancel_Map_LoadFile = true;
            return NULL;
		...
		else
		{
			...
			//++timo FIXME: access through the IShader object?
...
{
  if (!strlen(fa->texdef.name))
	{
#ifdef _DEBUG
		Sys_Printf("WARNING: unexpected texdef.name is empty in Brush.cpp CheckName\n");
#endif
		fa->texdef.SetName("radiant/notex");
		strcpy(pname, "radiant/notex");
		return;
	}

	// some manage to get long filename textures (with spaces) in their maps
	if (strchr( fa->texdef.name, ' ' ))
	{
		char Msg1[1024];
		sprintf( Msg1, "Can't save texture with spaces in name. Rename %s\nNOTE: This message may popup several times .. once for each buggy face detected.", fa->texdef.name );
		Sys_Printf("%s\n", Msg1 );
		MessageBox( g_qeglobals.d_hwndMain, Msg1, "Error saving map", MB_OK );
		strcpy( pname, "radiant/notex" );
		return;
	}
	...
	if (fa->texdef.name[0] == '(')
  {
		...
		Sys_Printf("%s\n", text);
		MessageBox( g_qeglobals.d_hwndMain, text, "Error saving map", MB_OK );
...
/*
      ...
      //++timo FIXME: bug #103494 detection attempt
      // TODO: clean this detection part when solved (remove below as well)
      if (pName[0] == '(')
      {
        char *text = "Bug #103494 detected, dropping texture. Please report to timo@qeradiant.com if you have a way to reproduce!";
        Sys_Printf("%s\n", text);
...
/*				pname = fa->texdef.name;
				...
				CheckName( fa, pname );
/*
				//++timo TODO: detect long file names (with spaces in) and refuse to save them

        //++timo FIXME: bug #103494 detection attempt
        // TODO: clean this detection part when solved (remove above as well)
        if (pname[0] == '(')
        {
          char *text = "Bug #103494 detected, dropping texture. Please report to timo@qeradiant.com if you have a way to reproduce!\nNOTE: this message may popup several times .. once for each buggy face detected.";
          Sys_Printf("%s\n", text);
          MessageBox( g_qeglobals.d_hwndMain, text, "Error saving map", MB_OK );
					// need to cleanup this dead face name or we may loop endlessly
					strcpy(fa->texdef.name, "radiant/notex");
          pname = "radiant/notex";
        } */
...
void SetFaceTexdef2 (brush_t *b, face_t *f, IShader *pShader, texdef_t *texdef, brushprimit_texdef_t *brushprimit_texdef, bool bFitScale, IPluginTexdef* pPlugTexdef) {
	...
	{
		f->texdef = *texdef;
		ConvertTexMatWithQTexture( brushprimit_texdef, NULL, &f->brushprimit_texdef, QERApp_Shader_ForName( f->texdef.name )->getTexture() );
	}
	else
		if (bFitScale)
		{
...
void Brush_BuildWindings( brush_t *b, bool bSnap )
{
    ...
    if (!g_bBuildWindingsNoTexBuild || !face->d_texture)
    {
      ...
      if (!face->d_texture && face->pShader)
        Sys_Printf("ERROR: unexpected face->pShader != NULL with face->d_texture == NULL in Brush_BuildWindings\n");
#endif
      if (face->pShader)
        face->pShader->DecRef();
      face->pShader = QERApp_Shader_ForName( face->texdef.name );
      face->pShader->IncRef();
...
void Brush_SnapToGrid(brush_t *pb)
{
  ...
  if (pb->owner->eclass->fixedsize == 1)
  {
    // snap the origin
    pb->owner->origin[0] = floor (pb->owner->origin[0] / g_qeglobals.d_gridsize + 0.5) * g_qeglobals.d_gridsize;
    pb->owner->origin[1] = floor (pb->owner->origin[1] / g_qeglobals.d_gridsize + 0.5) * g_qeglobals.d_gridsize;
    pb->owner->origin[2] = floor (pb->owner->origin[2] / g_qeglobals.d_gridsize + 0.5) * g_qeglobals.d_gridsize;
  }
  else
  {
    ...
    if (pModel != NULL && strlen(pModel) > 0)
    {
      ...
      if (e != NULL)
      {
        ...
        // recreate brush just like in load/save
		  ...
		  // JONATHAN: Add code to handle modelscale here for when the model is first loaded
		  char *ms_val = ValueForKey(b->owner, "modelscale");
		  if (ms_val != NULL)
		  {
			  float scale = FloatForKey(b->owner,"modelscale");
			  vec_t sc=scale;
			  if (sc>0)
			  {
				  VectorScale(vMin, sc, vMin);
				  VectorScale(vMax, sc, vMax);
			  }
		  }
#endif
...
bool PaintedModel(brush_t *b, bool bOkToTexture)
{
    ...
    if (pEclass)
    {
	  ...
	  // JONATHAN Check this section for angles + modelscale
      float a = FloatForKey (b->owner, "angle");
	  ...
	  // Pitch Yaw Roll
	  // Y  Z  X ?
	  // anticlockwise rotations when looking down axis
	  angles[0] = angles[1] = angles[2] = 0;
	  GetVectorForKey(b->owner,"angles",angles);
      ...
      while (model != NULL)
      {
        if (bOkToTexture == false || g_PrefsDlg.m_nEntityShowState & ENTITY_WIREFRAME || model->nTextureBind == -1)	// wireframe mode
        {
        ...
        else
        {
		  ...
		  qglEnable(GL_CULL_FACE); // JONATHAN : Need to turn these back on if they were turned off
		  qglPolygonMode (GL_FRONT_AND_BACK, GL_FILL); // jn added
		...
		// JONATHAN:: I should probably set b->owner->vRotation somewhere
        ...
        for (i = 0; i < model->nTriCount; i++)
        {
          for (j = 0; j < 3; j++)
          {
			...
			if (useAngles)
			{
				if (angles[2])				// This is X because angles takes arguments: Y Z X
				{
				  float y2 = (((y - v[1]) * vCos[2]) - ((z - v[2]) * vSin[2])) + v[1];
				  float z2 = (((y - v[1]) * vSin[2]) + ((z - v[2]) * vCos[2])) + v[2];
				  y = y2;
				  z = z2;
				}
				if (angles[0])
				{
				  float z2 = (((z - v[2]) * vCos[0]) - ((x - v[0]) * vSin[0])) + v[2];
				  float x2 = (((z - v[2]) * vSin[0]) + ((x - v[0]) * vCos[0])) + v[0];
				  x = x2;
				  z = z2;
				}
				if (angles[1])
				{
				  float x2 = (((x - v[0]) * vCos[1]) - ((y - v[1]) * vSin[1])) + v[0];
				  float y2 = (((x - v[0]) * vSin[1]) + ((y - v[1]) * vCos[1])) + v[1];
				  x = x2;
				  y = y2;
				}
			}
	  ...
	  // Silly box
        ...
        VectorCopy(b->mins, mins);
        VectorCopy(b->maxs, maxs);
		//VectorRotateEuler(mins,b->owner->origin,angles);
		//VectorRotateEuler(maxs,b->owner->origin,angles);
/*
		// JONATHAN: Rotate Bounding box
		// void VectorRotateEuler(vec3_t vertex,vec3_t pivot,vec3_t angles);
        if (a)
        {
          vec3_t vAngle;
          vAngle[0] = vAngle[1] = 0;
...
//	qglDisable( GL_TEXTURE_2D );
    ...
    {
			if (strstr(face->texdef.name, "clip"))
				continue;
		}
		...
		else if ((nDrawMode == cd_texture || nDrawMode == cd_light) && face->d_texture != prev)
		{
    ...
    // NOTE: alpha values when setting colors are only relevant if blending was enabled before calling Brush_Draw
    if (b->patchBrush)
