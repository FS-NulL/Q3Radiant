...
void CSurfaceDlg::GetTexMods()
{
  ...
  // TTimo: detect and refuse invalid texture names (at least the ones with spaces)
	if ((pt->name[0] <= ' ') || strchr(pt->name, ' '))
	{
		pt->SetName("radiant/notex");
		::SetWindowText(GetDlgItem(IDC_TEXTURE)->GetSafeHwnd(), pt->name);
	}
...
void CSurfaceDlg::UpdateSpinners(int nScrollCode, int nPos, CScrollBar* pBar)
{
  ...
  //++timo FIXME: what this part does is not very clear .. I'm not sur it even gets called
#ifdef _DEBUG
  Sys_Printf("FIXME: debug calling conditions and job done in CSurfaceDlg::UpdateSpinners\n");
#endif
...
#if 0
	//++timo FIXME: the FIT command is disabled in BP mode
	if (g_qeglobals.m_bBrushPrimitMode)
		g_dlgSurface.GetDlgItem( IDC_BTN_FACEFIT )->EnableWindow(FALSE);
	else
		g_dlgSurface.GetDlgItem( IDC_BTN_FACEFIT )->EnableWindow(TRUE);
#endif
}		
