...
CString g_pidFile;											// holds the full path to a Radiant.pid file we use to detect startup crashes
...
{
  //{"ViewGroups", 'G', 0x00, ID_VIEW_GROUPS}, //JONATHAN:: Removed due to not functioning
...
//  {"ToggleConsole", 'O', 0, ID_TOGGLECONSOLE},
  ...
  {"MatrixTranspose", 'M', 0x05, ID_CURVE_MATRIX_TRANSPOSE},
  ...
  {"SnapToGrid", 'G', 0x04, ID_SELECT_SNAPTOGRID},
  ...
  {"CSGMerge", 'M', 0x01, ID_SELECTION_CSGMERGE},
  {"RegionSetSelection", 'R', 0x05, ID_REGION_SETSELECTION},
  {"ShowInUse", 'U', 0, ID_TEXTURES_SHOWINUSE},
  {"ToggleShowHint", 'H', 0x04, ID_VIEW_SHOWHINT},
	{"InvertSelection", 'I', 0, ID_SELECTION_INVERT}
};
...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
  ...
  ON_COMMAND(ID_VIEW_CAMERATOGGLE, ToggleCamera)
	...
	ON_UPDATE_COMMAND_UI(ID_TEXTURES_SHOWALL, OnUpdateTexturesShowall)
	ON_COMMAND(IID_DONTSELECTMODEL, OnDontselectmodel)
	ON_COMMAND(ID_SELECTION_INVERT, OnSelectionInvert)
	ON_COMMAND(ID_TEXTURES_SHADERLISTONLY, OnTexturesShaderlistonly)
	//}}AFX_MSG_MAP
  ...
  // these are from CSurfaceDlg, shortcuts for FaceFit and PatchFit
	ON_COMMAND(IDC_BTN_FACEFIT, OnFaceFit)
...
void CMainFrame::SetButtonMenuStates()
{
  ...
  if (pMenu)
  {
    ...
    if (m_wndToolBar.GetSafeHwnd())
    {
      ...
      m_wndToolBar.GetToolBarCtrl().CheckButton(IID_DONTSELECTMODEL, (g_PrefsDlg.m_bSelectModels) ? FALSE : TRUE);
...
void CMainFrame::LoadCommandMap()
{
  ...
  for (int i = 0; i < g_nCommandCount; i++)
  {
    ...
    if (nLen > 0)
    {
			if (!bUserCmdList)
			{
				Sys_Printf("Found a user prefs command list...\n");
				bUserCmdList = true;
			}
...
void CMainFrame::CreateQEChildren()
{
  ...
  bool bProjectLoaded = false;
    ...
    Sys_Printf("Project file specified at the command line: %s\n", argv[1] );
	...
	else 
  {
    ...
    {
      Sys_Printf("Load last project %s\n", g_PrefsDlg.m_strLastProject.GetBuffer(0) );
    ...
    if (!bProjectLoaded)
    {
      ...
      Sys_Printf("Looking for default project at: %s\n", cWork );
      bProjectLoaded = QE_LoadProject(cWork);
    }
    if (!bProjectLoaded)
    {
      Sys_Printf("Desperate attempt: scripts/quake.qe4\n");
  ...
  if (!bProjectLoaded)
  {
    Sys_Printf("Prompting for project file\n");
...
void CMainFrame::OnBspCommand(unsigned int nID) 
{
  ...
  if (g_qeglobals.bBSPFrontendPlugin)
  {
	  ...
	  b+=c;
	  c+=b;
	  MessageBox("REACHED THE PROBLEM!"); // Jonathan: Disabled the next line, it doesn't compile and doesn't seem to ever run
	  //g_BSPFrontendTable.m_pfnDispatchBSPCommand( g_BSPFrontendCommands.GetAt(nID-CMD_BSPCOMMAND).GetBuffer((int)0) );
  }
  else
  {
...
void CMainFrame::OnViewZoomout() 
{
  ...
  if (m_pXYWnd && m_pXYWnd->Active())
  {
    ...
    if (m_pXYWnd->Scale() < 0.1)
      m_pXYWnd->SetScale(0.1);
  }

  if (m_pXZWnd && m_pXZWnd->Active())
  {
    m_pXZWnd->SetScale(m_pXZWnd->Scale() * 4.0 / 5);
    if (m_pXZWnd->Scale() < 0.1)
      m_pXZWnd->SetScale(0.1);
  }

  if (m_pYZWnd && m_pYZWnd->Active())
  {
    m_pYZWnd->SetScale(m_pYZWnd->Scale() * 4.0 / 5);
    if (m_pYZWnd->Scale() < 0.1)
      m_pYZWnd->SetScale(0.1);
  }
	Sys_UpdateWindows (W_XY|W_XY_OVERLAY);
}
...
{
  pCmdUI->SetCheck(!g_bShowAllShaders);
}
// show all check box
void CMainFrame::OnUpdateTexturesShowall(CCmdUI* pCmdUI) 
{
  pCmdUI->SetCheck(g_bShowAllShaders);
}
...
//++timo TODO: remove, now done with Shift+B "FitTexture"??
...
void CMainFrame::OnViewShowWorkzone()
{
	if (g_qeglobals.d_show_work)
	{
    CheckMenuItem ( ::GetMenu(g_qeglobals.d_hwndMain), ID_VIEW_SHOWWORKZONE, MF_BYCOMMAND | MF_UNCHECKED );
		g_qeglobals.d_show_work = false;
	}
	else
	{
    CheckMenuItem ( ::GetMenu(g_qeglobals.d_hwndMain), ID_VIEW_SHOWWORKZONE, MF_BYCOMMAND | MF_CHECKED );
...
void CMainFrame::OnDontselectmodel() 
{
  g_PrefsDlg.m_bSelectModels ^= 1;
  m_wndToolBar.GetToolBarCtrl().CheckButton(IID_DONTSELECTMODEL, (g_PrefsDlg.m_bSelectModels) ? FALSE : TRUE);
}

void CMainFrame::OnTexturesShaderlistonly() 
{
...
{
	if (zDelta>0) OnViewZoomin();
	else if (zDelta<0) OnViewZoomout();
	else return false;
	return true;
}*/