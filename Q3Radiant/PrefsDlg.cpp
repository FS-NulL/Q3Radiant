...
#define ALPHACHANNEL_KEY "NoAlphaChannel"
	...
	for (c=path+strlen(path)-1 ; c != path ; c--)
	{
      ...
      while (*sep && *sep != '/' && *sep != '\\')
      {
        ...
        count++;
			...
			strncpy (l_qdir, path, c+len+count-path);
			for ( i = 0; i < strlen( l_qdir ); i++ )
     {
				if ( l_qdir[i] == '\\' ) 
					l_qdir[i] = '/';
			}
			...
			while (*c)
			{
				if (*c == '/' || *c == '\\')
				{
					strncpy (l_gamedir, path, c+1-path);

					for ( i = 0; i < strlen( l_gamedir ); i++ )
					{
						if ( l_gamedir[i] == '\\' ) 
							l_gamedir[i] = '/';
					}
					...
					if ( !l_writedir[0] )
						strcpy( l_writedir, l_gamedir );
					else if ( l_writedir[strlen( l_writedir )-1] != '/' )
					{
						l_writedir[strlen( l_writedir )] = '/';
						l_writedir[strlen( l_writedir )+1] = 0;
					}
...
void CPrefsDlg::InitEnginePath()
{
	struct _finddata_t fileinfo;
  int handle;
  bool bPrompt = false;

  // JN TODO, this needs a major rewrite

  // try to guess the basepath
  // if we are going through a normal installation q3map is in the same directory than Radiant
  // and q3map needs SetQDirFromPath to operate
  // if this doesn't work to get the base dir we'll warn the user and browse for folder
  if ( SetQdirFromPath( g_strAppPath.GetBuffer(0) ) )
  {
    // look for quake3.exe in there
    m_strQuake2 = l_qdir;
    m_strQuake2 += "quake3.exe";
    handle = _findfirst (m_strQuake2.GetBuffer(0), &fileinfo);
    if (handle==-1)
    {
      MessageBox( "Could not find the path to the engine, please enter it in the following dialog", "Need path to the engine" );
      bPrompt = true;
    }
		_findclose (handle);
  }
  ...
  {
    MessageBox( "You need a a directory that contains \"quake\" above the tools. I can't find the path to the engine, please enter it in the following dialog.", "WARNING: tools may not work" );
    bPrompt = true;
  }
  // if fails, warn and browse:
  if (bPrompt)
  {
    CFileDialog dlg(true, NULL, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, "Executables (*.exe)|*.exe||", this);
    if (dlg.DoModal() == IDOK)
...
// TTimo: m_strQuake2 has a special status, if not found in registry we need to 
...
void CPrefsDlg::LoadPrefs()
{
  ...
  //++timo FIXME: remove broken Q2 support
  m_bHiColorTextures = TRUE;
//  m_bHiColorTextures = AfxGetApp()->GetProfileInt(PREF_SECTION, HICOLOR_KEY, 1);
  ...
  m_nUndoLevels = AfxGetApp()->GetProfileInt(PREF_SECTION, UNDOLEVELS_KEY, 30);
  m_bVertexSplit = AfxGetApp()->GetProfileInt(PREF_SECTION, VERTEXMODE_KEY, 1);
  m_bDisableAlphaChannel = AfxGetApp()->GetProfileInt(PREF_SECTION, ALPHACHANNEL_KEY, FALSE);
  m_bSelectCurves = AfxGetApp()->GetProfileInt(PREF_SECTION, SELECTCURVES_KEY, TRUE);
  m_bSelectModels = AfxGetApp()->GetProfileInt(PREF_SECTION, SELECTMODELS_KEY, TRUE);
  m_bTexturesShaderlistOnly = AfxGetApp()->GetProfileInt(PREF_SECTION, SHADERLISTONLY_KEY, FALSE);

  // if the log file is already on, we ignore the prefs
  if (!g_qeglobals.hLogFile)
    m_bLogConsole = AfxGetApp()->GetProfileInt(PREF_SECTION, LOGCONSOLE_KEY, FALSE);
...
void CPrefsDlg::SavePrefs()
{
  ...
  if (m_nMouse == 0)
    ...
    m_nMouseButtons = 3;
  AfxGetApp()->WriteProfileInt(PREF_SECTION, "LastLightIntensity", m_iLastLightIntensity);
  ...
  AfxGetApp()->WriteProfileInt(PREF_SECTION, VERTEXMODE_KEY, m_bVertexSplit);
  AfxGetApp()->WriteProfileInt(PREF_SECTION, LOGCONSOLE_KEY, m_bLogConsole);
  AfxGetApp()->WriteProfileInt(PREF_SECTION, ALPHACHANNEL_KEY, m_bDisableAlphaChannel);
  AfxGetApp()->WriteProfileInt(PREF_SECTION, SELECTCURVES_KEY, m_bSelectCurves);
  AfxGetApp()->WriteProfileInt(PREF_SECTION, SELECTMODELS_KEY, m_bSelectModels);
  AfxGetApp()->WriteProfileInt(PREF_SECTION, SHADERLISTONLY_KEY, m_bTexturesShaderlistOnly);
}
...
void CPrefsDlg::SetGamePrefs()
{
  ...
  {
    //++timo FIXME: Q2 support is broken and needs to go away (sort of)
    Sys_Printf("WARNING: unknown game prefs .. expect anything.\n");
...
void CPrefsDlg::OnButtonClean() 
{
  ...
  if (MessageBox( "This will close Radiant and clean the corresponding registry entries. Next time you start Radiant it will be good as new. Do you wish to continue?", "Reset Registry", MB_YESNO ) == IDYES )
  {
