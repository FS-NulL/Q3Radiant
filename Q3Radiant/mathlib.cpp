...
void VectorRotateEuler(vec3_t in,vec3_t v,vec3_t angles)
{
	// ANGLES in YZX Format
	vec3_t vSin,vCos;
	int j;
	for ( j = 0; j < 3; j++)
    {
	...
	if (angles[2])
	{
	  float y2 = (((in[1] - v[1]) * vCos[2]) - ((in[2] - v[2]) * vSin[2])) + v[1];
	  float z2 = (((in[1] - v[1]) * vSin[2]) + ((in[2] - v[2]) * vCos[2])) + v[2];
	  in[1] = y2;
	  in[2] = z2;
	}
	if (angles[0])
	{
	  float z2 = (((in[2] - v[2]) * vCos[0]) - ((in[0] - v[0]) * vSin[0])) + v[2];
	  float x2 = (((in[2] - v[2]) * vSin[0]) + ((in[0] - v[0]) * vCos[0])) + v[0];
	  in[0] = x2;
	  in[2] = z2;
	}
	if (angles[1])
	{
	  float x2 = (((in[0] - v[0]) * vCos[1]) - ((in[1] - v[1]) * vSin[1])) + v[0];
	  float y2 = (((in[0] - v[0]) * vSin[1]) + ((in[1] - v[1]) * vCos[1])) + v[1];
	  in[0] = x2;
	  in[1] = y2;
	}
}