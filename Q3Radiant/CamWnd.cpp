...
void CCamWnd::Cam_Draw()
{
	...
	qglPushMatrix(); // gdewan: cheap hack so as not to alter Cam_BuildMatrix() code
	...
	qer_cam_x = m_Camera.origin[0];
	qer_cam_y = m_Camera.origin[1];
	qer_cam_z = m_Camera.origin[2];
	qer_cam_yaw = m_Camera.angles[0];
	qer_cam_pitch = m_Camera.angles[1];
...
/*	case cd_blend:
    ...
    if (brush->brush_faces->pShader->getFlags() & QER_TRANS)
