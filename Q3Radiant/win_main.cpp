...
void QE_ExpandBspString (char *bspaction, char *out, char *mapname, bool useTemps)
{
  ...
  QE_ConvertDOSToUnixName(src, src);
	...
	in = ValueForKey( g_qeglobals.d_project_entity, bspaction ); // Jonathan note: the comiple command gets collected here
	while (*in)
	{
		if (in[0] == '!')
		{
...
CTime g_tBegin;
void RunBsp (char *command) // Jonathan TODO: should be able to get the fs_basepath parameter from the projectsettings entity  (g_qeglobals.d_project_entity)
{                           // Jonathan TODO: char *rsh = ValueForKey(g_qeglobals.d_project_entity, "basepath"); //?
	char	sys[2048];
	char	batpath[2048];
